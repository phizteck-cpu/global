...
router.get('/pending-registrations', authenticateToken, anyAdmin, async (req, res) => {
    try {
        const pendingUsers = await prisma.user.findMany({
            where: { 
...
router.post('/registrations/:id/approve', authenticateToken, anyAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const adminId = req.user.userId;
        ...
        await prisma.auditLog.create({
            data: {
                ...
                details: `Registration approved for ${updatedUser.username}`
            }
        });

        res.json({ message: 'Registration approved successfully', user: updatedUser });
    } catch (error) {
        console.error(error);
...
router.post('/registrations/:id/reject', authenticateToken, anyAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const { reason } = req.body;
        const adminId = req.user.userId;
        ...
        await prisma.auditLog.create({
            data: {
                ...
                targetUserId: updatedUser.id,
                details: `Registration rejected for ${updatedUser.username}. Reason: ${reason}`
            }
        });

        res.json({ message: 'Registration rejected', user: updatedUser });
    } catch (error) {
        console.error(error);
...
router.post('/registrations/bulk-approve', authenticateToken, anyAdmin, async (req, res) => {
    try {
        ...
        await prisma.auditLog.create({
            data: {
                adminId,
                action: 'BULK_REGISTRATION_APPROVED',
                details: `Bulk approved ${updatedUsers.count} registrations`
            }
        });

        res.json({ message: `${updatedUsers.count} registrations approved successfully` });
    } catch (error) {
        console.error(error);
...
// GET /api/admin/pending-withdrawals - Get all pending withdrawals
router.get('/pending-withdrawals', authenticateToken, anyAdmin, async (req, res) => {
    try {
...
router.post('/withdrawals/:id/approve', authenticateToken, anyAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const { adminNote } = req.body;
        const adminId = req.user.userId;
        ...
        const [updatedWithdrawal] = await prisma.$transaction([
            ...
            prisma.transaction.create({
                data: {
                    ...
                    amount: withdrawal.amount,
                    status: 'SUCCESS',
                    reference: `WTH-${id}-${Date.now()}`,
                    description: `Withdrawal approved: ${adminNote || 'Approved'}`
                }
            }),
            ...
            prisma.auditLog.create({
                data: {
                    ...
                    targetUserId: withdrawal.userId,
                    details: `Approved withdrawal #${id} of ₦${withdrawal.amount}. ${adminNote || ''}`
                }
            })
        ]);
...
router.post('/withdrawals/:id/reject', authenticateToken, anyAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const { reason } = req.body;
        const adminId = req.user.userId;
        ...
        await prisma.notification.create({
            data: {
                ...
                message: `Your withdrawal request of ₦${withdrawal.amount.toLocaleString()} has been rejected. Reason: ${reason}`
            }
        });
        ...
        await prisma.auditLog.create({
            data: {
                ...
                targetUserId: withdrawal.userId,
                details: `Rejected withdrawal #${id} of ₦${withdrawal.amount}. Reason: ${reason}`
            }
        });

        res.json({ message: 'Withdrawal rejected', withdrawal: updatedWithdrawal });
    } catch (error) {
        console.error(error);
...
// POST /api/admin/withdrawals/:id/process - Mark withdrawal as processed/completed
router.post('/withdrawals/:id/process', authenticateToken, anyAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const { adminNote } = req.body;
        const adminId = req.user.userId;
        ...
        await prisma.notification.create({
            data: {
                ...
                message: `Your withdrawal of ₦${withdrawal.amount.toLocaleString()} has been processed and sent to your bank.`
            }
        });
        ...
        await prisma.auditLog.create({
            data: {
                ...
                targetUserId: withdrawal.userId,
                details: `Completed withdrawal #${id} of ₦${withdrawal.amount}`
            }
        });

        res.json({ message: 'Withdrawal completed', withdrawal: updatedWithdrawal });
    } catch (error) {
        console.error(error);
...
// GET /api/admin/withdrawal-stats - Get withdrawal statistics
router.get('/withdrawal-stats', authenticateToken, anyAdmin, async (req, res) => {
    try {
        ...
        res.json({
            ...
            rejected,
            completed,
            totalProcessed: totalAmount._sum.amount || 0
        });
    } catch (error) {
        console.error(error);
