                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {proofs.map((proof) => (
                        <div key={proof.id} className="bg-surfaceHighlight rounded-3xl p-6 border border-white/5">
                            ...
                            <div className="mb-4">
                                <img
                                    src={`${import.meta.env.VITE_API_URL?.replace('/api', '') || ''}/${proof.proofImageUrl}`}
                                    alt="Payment Proof"
                                    className="w-full h-48 object-cover rounded-xl border border-white/10 cursor-pointer hover:opacity-80 transition-opacity"
                                    onClick={() => window.open(`${import.meta.env.VITE_API_URL?.replace('/api', '') || ''}/${proof.proofImageUrl}`, '_blank')}
                                />
                            </div>
