...
const Signup = () => {
    ...
    const handleSignup = async (e) => {
        ...
        try {
            // Map sponsorUsername to referralCode for backend
            const signupData = {
                ...formData,
                referralCode: formData.sponsorUsername || null
            };
            delete signupData.sponsorUsername;
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 px-4 py-12">
            <div className="max-w-2xl w-full bg-sky-100 backdrop-blur-xl p-8 rounded-2xl shadow-xl border border-sky-200">
                {/* Step Indicator */}
                ...
                {step === 1 && (
                    <>
                        <div className="text-center mb-8">
                            ...
                            <p className="text-slate-700 font-semibold">Join Valuehills and start your journey</p>
                        </div>
                        ...
                        <form onSubmit={handleSignup} className="space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        name="firstName"
                                        required
                                        className="w-full px-4 py-3 rounded-lg border border-slate-300 focus:ring-2 focus:ring-brand-green/20 focus:border-brand-green outline-none transition-all bg-white font-bold text-slate-900"
                                        placeholder="John"
                            ...
                            {/* Tier Selection */}
                            <div>
                                <label className="block text-sm font-bold text-slate-800 mb-3">Select Membership Tier</label>
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                    {tiers.map((tier) => (
                                        <div key={tier.id}
                                            onClick={() => setFormData({ ...formData, tierId: tier.id })}
                                            className={`cursor-pointer p-4 rounded-xl border-2 transition-all ${parseInt(formData.tierId) === tier.id ? 'border-brand-green bg-green-50' : 'border-slate-300 hover:border-slate-400'}`}>
                                            <h3 className="font-bold text-slate-900">{tier.name}</h3>
                                            <p className="text-sm text-slate-600">₦{tier.weeklyAmount.toLocaleString()}/wk</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="bg-sky-50 p-4 rounded-xl border border-sky-200">
                                <label className="block text-sm font-bold text-slate-800 mb-1">
                                    Sponsor Username <span className="text-slate-500 font-normal">(Optional)</span>
                                </label>
                                <p className="text-xs text-slate-600 mb-3">Enter the username of the person who referred you or your team leader</p>
                                <input
                                    type="text"
                                    name="sponsorUsername"
                                    className="w-full px-4 py-3 rounded-lg border border-slate-300 focus:ring-2 focus:ring-brand-green/20 focus:border-brand-green outline-none transition-all bg-white font-bold text-slate-900"
                                    placeholder="e.g., johnsmith or leave blank"
                                    onChange={handleChange}
                                />
                            </div>
                        ...
                        <p className="mt-6 text-center text-slate-700 font-semibold">
