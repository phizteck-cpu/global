...
const AdminUsers = () => {
    ...
    const handleViewProfile = async (user) => {
        ...
        try {
            const res = await axiosClient.get(`/admin/users/${user.id}`);
    ...
    const handleUpdateStatus = async (status, kycVerified) => {
        ...
        try {
            await axiosClient.patch(`/admin/users/${selectedUser.id}/status`, { status, kycVerified });
            fetchUsers();
            setShowActions(false);
            if (showProfile) {
                const res = await axiosClient.get(`/admin/users/${selectedUser.id}`);
                setProfileData(res.data);
            }
        } catch (error) {
    ...
    const handleBalanceAdjustment = async (e) => {
        ...
        try {
            ...
            await axiosClient.patch(`/admin/users/${selectedUser.id}/balance`, data);
    ...
    const handleManualTierUpdate = async () => {
        ...
        setActionLoading(true);
        try {
            await axiosClient.patch(`/admin/users/${selectedUser.id}/tier`, { tierId: selectedTierId });
            fetchUsers();
            if (showProfile) handleViewProfile(selectedUser);
                                ...
                                <tr key={u.id} className="group hover:bg-white/[0.02] transition-all">
                                    ...
                                    <td className="px-10 py-8 text-right">
                                        <div className="space-y-1">
                                            ...
                                            <p className="text-[10px] uppercase tracking-[0.2em] text-noble-gray font-black italic">Locked CaxiosClienttal</p>
                                        </div>
                                    </td>
                                    <td className="px-10 py-8">
                        ...
                        >
                            ...
                            <div className="p-10 overflow-y-auto space-y-12">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                    ...
                                    <ProfileDetailCard label="Cooperative Locked" value={`₦${profileData.contributionBalance?.toLocaleString()}`} sub="Secured Institutional CaxiosClienttal" icon={<ShieldCheck size={20} />} color="text-primary" />
